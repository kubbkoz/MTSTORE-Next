
import React, { useState, useEffect } from 'react';
import { X, Ruler } from 'lucide-react';
import Button from './Button';

interface SizeChartModalProps {
  isOpen: boolean;
  onClose: () => void;
  category: string;
  availableSizes?: string[];
}

const SizeChartModal: React.FC<SizeChartModalProps> = ({ isOpen, onClose, category, availableSizes }) => {
  // Height in cm
  const [riderHeight, setRiderHeight] = useState<number>(178);
  const [activeSize, setActiveSize] = useState<string>('M');

  // Logic to map height to size
  useEffect(() => {
    let calculatedSize = 'M';
    if (riderHeight < 170) calculatedSize = 'XS';
    else if (riderHeight >= 170 && riderHeight < 180) calculatedSize = 'S';
    else if (riderHeight >= 180 && riderHeight < 185) calculatedSize = 'M';
    else if (riderHeight >= 185 && riderHeight < 195) calculatedSize = 'L';
    else calculatedSize = 'XL';

    // If we have specific available sizes, try to select the calculated one, 
    // otherwise default to the first available one to avoid showing empty data
    if (availableSizes && availableSizes.length > 0) {
      if (availableSizes.includes(calculatedSize)) {
        setActiveSize(calculatedSize);
      } else {
        // Simple fallback logic: find closest or just pick first
        // For better UX, we could map ranges, but here we'll ensure we don't show a size tab that doesn't exist
        // If calculated size is not in available, check if we should switch to one that is
         if (!availableSizes.includes(activeSize)) {
             setActiveSize(availableSizes[0]);
         }
      }
    } else {
      setActiveSize(calculatedSize);
    }
  }, [riderHeight, availableSizes]);

  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === 'Escape') onClose();
    };
    window.addEventListener('keydown', handleEsc);
    return () => window.removeEventListener('keydown', handleEsc);
  }, [onClose]);

  if (!isOpen) return null;

  // Mock Data for Geometry based on active size
  const geometryData: Record<string, Record<string, string>> = {
    'XS': { 'Wheel Size': '27.5"', 'Seat Tube': '440', 'Top Tube': '550', 'Head Tube': '100', 'Reach': '390', 'Stack': '590', 'Wheelbase': '1140' },
    'S': { 'Wheel Size': '29"', 'Seat Tube': '480', 'Top Tube': '580', 'Head Tube': '110', 'Reach': '420', 'Stack': '605', 'Wheelbase': '1170' },
    'M': { 'Wheel Size': '29"', 'Seat Tube': '503', 'Top Tube': '605', 'Head Tube': '120', 'Reach': '450', 'Stack': '615', 'Wheelbase': '1200' },
    'L': { 'Wheel Size': '29"', 'Seat Tube': '520', 'Top Tube': '630', 'Head Tube': '130', 'Reach': '475', 'Stack': '630', 'Wheelbase': '1230' },
    'XL': { 'Wheel Size': '29"', 'Seat Tube': '540', 'Top Tube': '650', 'Head Tube': '145', 'Reach': '500', 'Stack': '645', 'Wheelbase': '1260' },
  };

  const allSizes = ['XS', 'S', 'M', 'L', 'XL'];
  
  // Filter sizes to display
  const displaySizes = availableSizes && availableSizes.length > 0 
    ? allSizes.filter(s => availableSizes.includes(s))
    : allSizes;

  const sizeLabels: Record<string, string> = {
    'XS': 'XS 49 (155-170cm)',
    'S': 'S 52 (170-180cm)',
    'M': 'M 54 (175-185cm)',
    'L': 'L 56 (180-190cm)',
    'XL': 'XL 58 (185-200cm)',
  };

  // Fallback if activeSize somehow isn't in geometryData (e.g. custom sizes)
  const currentGeo = geometryData[activeSize] || geometryData['M'];

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 font-sans">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity" onClick={onClose} />
      <div className="relative w-full max-w-5xl bg-white shadow-2xl animate-slide-up rounded-none overflow-hidden flex flex-col max-h-[90vh]">
        
        {/* Header */}
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <div>
            <h2 className="text-2xl font-black uppercase font-tech tracking-wide mb-1 flex items-center">
              <Ruler className="w-6 h-6 mr-2 text-brand" /> TABUĽKA VEĽKOSTÍ A GEOMETRIA RÁMU
            </h2>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-gray-200 transition-colors rounded-none">
            <X className="w-6 h-6" />
          </button>
        </div>

        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 md:p-10">
          <div className="flex flex-col lg:flex-row gap-12">
            
            {/* Left Side: Visuals & Slider */}
            <div className="flex-1">
              {/* Bike Geometry Image */}
              <div className="bg-gray-50 p-6 mb-8 border border-gray-100 flex items-center justify-center">
                <img 
                  src="https://www.mt-sport.sk/wp-content/uploads/2025/07/image-58.png.webp" 
                  alt="Geometry Diagram" 
                  className="w-full h-auto max-h-[300px] object-contain mix-blend-multiply"
                />
              </div>

              {/* Rider Height Slider */}
              <div className="bg-gray-50 p-6 border border-gray-100">
                <label className="block text-sm font-bold uppercase tracking-wider text-gray-500 mb-4 text-center">
                  Nastavte výšku jazdca: <span className="text-brand text-lg">{riderHeight} cm</span>
                </label>
                <div className="relative px-2">
                  <input 
                    type="range" 
                    min="155" 
                    max="205" 
                    value={riderHeight} 
                    onChange={(e) => setRiderHeight(parseInt(e.target.value))}
                    className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-brand"
                  />
                  <div className="flex justify-between text-xs text-gray-400 mt-2 font-mono font-bold">
                    <span>155cm</span>
                    <span>180cm</span>
                    <span>205cm</span>
                  </div>
                </div>
                <div className="mt-6 text-center">
                  <span className="text-sm text-gray-600">Odporúčaná veľkosť:</span>
                  <div className="text-4xl font-black font-tech text-black mt-1">
                    {activeSize}
                    {availableSizes && availableSizes.length > 0 && !availableSizes.includes(activeSize) && (
                       <span className="block text-xs text-red-500 font-sans mt-1">(Momentálne nedostupné)</span>
                    )}
                  </div>
                </div>
              </div>
            </div>

            {/* Right Side: Data Table */}
            <div className="flex-1">
              {/* Size Tabs */}
              <div className="flex flex-wrap gap-2 mb-8 border-b border-gray-200 pb-4">
                {displaySizes.map((size) => (
                  <button
                    key={size}
                    onClick={() => setActiveSize(size)}
                    className={`px-4 py-2 text-sm font-bold uppercase tracking-wider transition-all border-b-4 ${
                      activeSize === size 
                        ? 'border-brand text-black bg-gray-50' 
                        : 'border-transparent text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    {size}
                  </button>
                ))}
              </div>

              <h3 className="text-xl font-bold uppercase font-tech mb-6 text-center lg:text-left">
                {sizeLabels[activeSize] || activeSize}
              </h3>

              {/* Geometry Grid */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4">
                {Object.entries(currentGeo).map(([key, value]) => (
                  <div key={key} className="text-center">
                    <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{key}</span>
                    <span className="block text-xl font-black font-tech text-gray-900">{value} {key === 'Wheel Size' ? '' : 'mm'}</span>
                  </div>
                ))}
                
                {/* Static filler data for visuals */}
                <div className="text-center">
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">BB Drop</span>
                  <span className="block text-xl font-black font-tech text-gray-900">70 mm</span>
                </div>
                 <div className="text-center">
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Seat Angle</span>
                  <span className="block text-xl font-black font-tech text-gray-900">74°</span>
                </div>
                 <div className="text-center">
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Head Angle</span>
                  <span className="block text-xl font-black font-tech text-gray-900">68°</span>
                </div>
                <div className="text-center">
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Chainstay</span>
                  <span className="block text-xl font-black font-tech text-gray-900">435 mm</span>
                </div>
              </div>

              <div className="mt-12 p-4 bg-blue-50 border border-blue-100 text-sm text-blue-800 rounded-none">
                <p><strong>Tip experta:</strong> Ak sa nachádzate na rozmedzí dvoch veľkostí, pre športovejšiu jazdu zvoľte menší rám, pre komfortnejšiu jazdu zvoľte väčší rám.</p>
              </div>
            </div>

          </div>
        </div>

         {/* Footer */}
         <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end">
            <Button variant="ghost" onClick={onClose} className="uppercase text-xs font-bold tracking-widest">Zatvoriť</Button>
         </div>
      </div>
    </div>
  );
};

export default SizeChartModal;
